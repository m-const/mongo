pipeline {
    ...
    stages{
        ...
        stage("Deploy"){
            steps{
                sh "docker run --name ${params.CONTAINER_NAME} -v ${params.VOLUME}:/etc/mongo -p ${PORT}:27017 -d ${params.IMAGE_NAME}-mongo:${DOCKER_IMAGE_VERSION} --restart unless-stopped --config /etc/mongo/mongod.conf"
            }
        }
